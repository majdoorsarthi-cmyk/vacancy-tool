...
// 📥 फॉर्म सबमिशन हैंडलिंग (PDF Form Special - Sent to Admin for Review)
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if ($is_suspended == 1) {
        $message = "<div class='alert error'>❌ आपका अकाउंट ब्लॉक है। आप नया PDF फॉर्म पोस्ट नहीं कर सकते।</div>";
    } else {
        $title = isset($_POST['title']) ? trim($_POST['title']) : '';
        $category = 'pdf_form'; // 📑 PDF फॉर्म कैटेगरी फिक्स रखी गई है
        $state = isset($_POST['state']) ? $_POST['state'] : ''; 
        $short_desc = isset($_POST['short_desc']) ? trim($_POST['short_desc']) : '';
        $result_link = isset($_POST['result_link']) ? trim($_POST['result_link']) : ''; // Rules / Guidelines PDF Link
        $long_details = isset($_POST['long_details']) ? $_POST['long_details'] : ''; // Download & Submission Process
        $description = isset($_POST['description']) ? $_POST['description'] : ''; // Required Documents
        $important_dates = isset($_POST['important_dates']) ? trim($_POST['important_dates']) : '';
        $eligibility = isset($_POST['eligibility']) ? trim($_POST['eligibility']) : ''; // Eligibility Criteria
        $application_fee = isset($_POST['application_fee']) ? trim($_POST['application_fee']) : '';
        $apply_link = isset($_POST['apply_link']) ? trim($_POST['apply_link']) : ''; // Direct PDF Download Link
        $official_website = isset($_POST['official_website']) ? trim($_POST['official_website']) : '';
        $last_date = !empty($_POST['last_date']) ? $_POST['last_date'] : NULL; // Submission Last Date
        $total_posts = !empty($_POST['total_posts']) ? trim($_POST['total_posts']) : 'N/A';
        
        // 🔒 सुरक्षा: हमेशा एडमिन वेरिफिकेशन हेतु 'pending' रहेगा (सीधे लाइव नहीं होगा)
        $status = 'pending'; 
        
        $full_details = isset($_POST['full_details']) ? $_POST['full_details'] : ''; // Office Address & Contact Details
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: pdf.php?success=1&rankmsg=" . urlencode($rank_updated_msg));
                exit;
            } else {
...
if (isset($_GET['success'])) {
    ...
    $message = "<div class='alert success'>⏳ PDF फॉर्म सफलतापूर्वक एडमिन वेरिफिकेशन (Approval) के लिए भेज दिया गया है! एडमिन की समीक्षा के बाद यह लाइव हो जाएगा। (+10 पॉइंट्स प्राप्त हुए){$rank_msg}</div>";
}
?>
<!DOCTYPE html>
<html lang="hi">
<head>
    ...
    <title>नया PDF फॉर्म जोड़ें | ऑपरेटर डैशबोर्ड</title>
    ...
    <div class="hidden md:flex flex-col w-64 bg-[#0f1423] text-slate-400 min-h-screen border-r border-slate-800/40 shrink-0 justify-between p-4">
        <div>
            <div class="flex items-center gap-3 px-2 py-4 mb-4 border-b border-slate-800">
                <div class="h-10 w-10 rounded-xl bg-rose-600 flex items-center justify-center text-white shadow-lg">
            ...
            <nav class="space-y-1.5">
                ...
                <a href="pdf.php" class="flex items-center gap-3.5 px-4 py-3 rounded-xl text-[13px] font-semibold bg-rose-600/15 text-rose-400 border-l-4 border-rose-500 transition-all">
                ...
                <a href="manage_vacancies.php" class="flex items-center gap-3.5 px-4 py-3 rounded-xl text-[13px] font-medium hover:bg-slate-900 hover:text-slate-200 transition-all">
                    <i class="fas fa-list-check text-sm w-4 text-center"></i> <span>वैकेंसी/फॉर्म्स प्रबंधित करें</span>
                </a>
            </nav>
        </div>
    ...
    <main class="flex-grow p-8 box-sizing-border overflow-x-hidden">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <h2 class="text-2xl font-bold text-slate-800 flex items-center gap-2">
                <i class="fas fa-file-pdf text-rose-600"></i> नया PDF फॉर्म जोड़ें (ऑपरेटर डैशबोर्ड)
            </h2>
        ...
        <div class="admin-container">
            <form method="POST">
                ...
                <div class="form-group">
                    <label>PDF Form / Scheme Title (PDF फॉर्म/योजना का नाम): *</label>
                    <input type="text" name="title" required placeholder="जैसे: MP Income Certificate Application Form PDF 2026 या Ladli Behna Yojana Offline Form PDF" <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>>
                </div>

                <div class="form-row">
                    <div>
                        <label>Category (कैटेगरी):</label>
                        <select name="category" disabled class="bg-slate-200 font-bold text-rose-800">
                            <option value="pdf_form" selected>📑 Pdf फॉर्म्स (PDF Forms)</option>
                        </select>
                    </div>
                    <div>
                ...
                <div class="form-row">
                    <div>
                        <label>Last Date / Submission Deadline (अंतिम तिथि - यदि लागू हो):</label>
                        <input type="date" name="last_date" <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>>
                    </div>
                    <div>
                        <label>Issuing Authority / Department (विभाग का नाम):</label>
                        <input type="text" name="total_posts" placeholder="जैसे: राजस्व विभाग / लोक सेवा केंद्र" <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>>
                    </div>
                    <div>
                        <label>Form Fee / Stamp Duty (शुल्क / चालान):</label>
                        <input type="text" name="application_fee" placeholder="जैसे: निःशुल्क / ₹10 कोर्ट फीस स्टाम्प" <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>>
                    </div>
                </div>

                <div class="form-group">
                    <label>Short Overview (संक्षिप्त सारांश):</label>
                    <input type="text" name="short_desc" placeholder="जैसे: आय प्रमाण पत्र हेतु ऑफलाइन आवेदन पत्र PDF यहाँ से डाउनलोड करें और तहसील में जमा करें..." <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>>
                </div>

                <!-- 2. विस्तृत विवरण (CKEditors) -->
                <div class="form-group">
                    <label>फॉर्म डाउनलोड और जमा करने की पूरी प्रक्रिया (Step by Step Form Download & Submission Process):</label>
                    <textarea name="long_details" id="editor1" <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>></textarea>
                </div>

                <div class="form-group">
                    <label>आवश्यक दस्तावेज एवं संलग्नक (Required Supporting Documents):</label>
                    <textarea name="description" id="editor2" <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>></textarea>
                </div>

                <div class="form-group">
                    <label>फॉर्म जमा करने का कार्यालय / पता (Submission Office & Address Details):</label>
                ...
                <div class="form-row">
                    <div>
                        <label>पात्रता एवं नियम (Eligibility Criteria):</label>
                        <textarea name="eligibility" rows="4" placeholder="जैसे: 
1. आवेदक राज्य का स्थायी निवासी होना चाहिए।
2. परिवार की वार्षिक आय निर्धारित सीमा के भीतर हो।" <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>></textarea>
                    </div>
                    <div>
                        <label>महत्वपूर्ण तिथियां (Important Timeline):</label>
                        <textarea name="important_dates" rows="4" placeholder="जैसे: 
• फॉर्म जारी तिथि: 01/01/2026
• अंतिम तिथि: जारी नहीं (सतत प्रक्रिया)
• जारी होने की अवधि: 7 से 15 कार्यदिवस" <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>></textarea>
                    </div>
                </div>

                <!-- 4. डायरेक्ट PDF डाउनलोड एवं ऑफिसियल लिंक्स -->
                <div class="form-row">
                    <div>
                        <label>Direct PDF Form Download Link: *</label>
                        <input type="text" name="apply_link" required placeholder="https://..." <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>>
                    </div>
                    <div>
                        <label>Official Rules / Guidelines PDF Link:</label>
                ...
                <button type="submit" class="btn-submit" <?php echo ($is_suspended == 1) ? 'disabled' : ''; ?>>
                    📤 PDF फॉर्म एडमिन समीक्षा (Verification) के लिए भेजें
                </button>
            </form>
        </div>
    </main>
...
<div id="guideModal" class="fixed inset-0 bg-black/60 z-50 hidden items-center justify-center p-4 backdrop-blur-sm">
    <div class="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200">
        <h3 class="text-lg font-bold text-slate-800 mb-3 flex items-center gap-2">
            <i class="fas fa-info-circle text-sky-600"></i> PDF फॉर्म पोस्ट करने के निर्देश
        </h3>
        <ul class="list-disc list-inside space-y-2 text-sm text-slate-600">
            <li><strong>Title:</strong> फॉर्म/योजना का नाम स्पष्ट दर्ज करें।</li>
            <li><strong>Direct Link:</strong> 'Direct PDF Download Link' में सीधा फाइल का URL भरें ताकि यूजर सिंगल क्लिक में डाउनलोड कर सके।</li>
            <li><strong>Required Documents:</strong> फॉर्म भरने के लिए किन-किन डॉक्यूमेंट्स की जरूरत होगी, उनकी सूची स्पष्ट रूप से लिखें।</li>
        </ul>
    ...
    <div class="bg-white rounded-2xl max-w-3xl w-full p-6 shadow-2xl border border-slate-200 text-slate-800">
        ...
        <div class="space-y-3 text-sm text-slate-600 max-h-[70vh] overflow-y-auto pr-2">
            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200">
                ...
                <p>आपके द्वारा सबमिट किया गया यह फॉर्म सीधा एडमिन वेरिफिकेशन में जाएगा। एडमिन की समीक्षा के बाद यह पब्लिकली लाइव होगा।</p>
            </div>
            <div class="p-3 bg-slate-50 rounded-xl border border-slate-200">
                <strong class="text-indigo-600 block mb-1">2. तुरंत अंक (+10 Points):</strong>
                <p>सबमिट करते ही ऑपरेटर खाते में तुरंत +10 पॉइंट्स क्रेडिट हो जाएंगे!</p>
            </div>
        </div>
        <div class="mt-6 text-right">
